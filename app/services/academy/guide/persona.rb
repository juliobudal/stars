...
module Academy
  module Guide
    # System prompt voice for "Pergunta ao Guia" — kid-side post-mission
    # Q&A with the LLM. Each VERSION's body is frozen at conversation
    # creation time via `GuideConversation#prompt_version` so a transcript's
    # meaning stays interpretable years later, even after the prompt evolves.
    #
    # The current active version is `VERSION`. v1 lives in `VOICE_V1` for
    # archival / replay; new conversations get `VOICE` (always pointing at
    # the latest).
    #
    # Versioning:
    ...
    module Persona
      VERSION = "guide-persona@v2"

      # v2 — dynamic-context aware. The system prompt now ships with four
      # tailored blocks (conceito, lentes recentes com cenas concretas,
      # estado do aprendiz, momento). The voice section instructs how to
      # USE those anchors instead of reciting them.
      VOICE_V2 = <<~PROMPT
