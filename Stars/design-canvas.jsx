...
function DesignCanvas({ children, minScale, maxScale, style, initialFocus }) {
  ...
  React.useEffect(() => {
    ...
    fetch('./' + DC_STATE_FILE)
      ...
      .catch(() => {})
      .finally(() => { didRead.current = true; if (!off) { if (initialFocus) setState(s => ({ ...s, focus: initialFocus })); setReady(true); } });
    const t = setTimeout(() => { if (!off) { if (initialFocus) setState(s => ({ ...s, focus: initialFocus })); setReady(true); } }, 150);
