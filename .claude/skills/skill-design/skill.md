...
Define colors as semantic tokens, not raw values. Use the naming convention where each role has a `-foreground` pair for guaranteed contrast:

```css
:root {
  --background: oklch(1 0 0);
  --foreground: oklch(0.145 0 0);
  --primary: oklch(0.205 0.015 285);
  --primary-foreground: oklch(0.985 0 0);
  --muted: oklch(0.965 0.003 264);
  --muted-foreground: oklch(0.556 0.015 264);
  --destructive: oklch(0.577 0.245 27);
  --border: oklch(0.922 0.004 264);
  --ring: oklch(0.708 0.005 264);
}
```

Every surface color gets a `-foreground` counterpart — this guarantees accessible contrast pairs by design. Components reference `var(--primary)` for background and `var(--primary-foreground)` for text, never hardcoded values.

For complex design systems needing fine-grained control across states (hover, active, focus), consider Radix UI's 12-step color scale as an alternative.
...
### The 4px Grid
All spacing uses a 4px base grid, defined as design tokens:

```css
--space-1: 4px;   /* micro — icon gaps */
--space-2: 8px;   /* tight — within components */
--space-3: 12px;  /* standard — between related elements */
--space-4: 16px;  /* comfortable — section padding */
--space-6: 24px;  /* generous — between sections */
--space-8: 32px;  /* major separation */
```

Any spacing value used more than once must be a CSS custom property — never hardcode raw pixels.

### Symmetrical Padding
**TLBR must match.** If top padding is 16px, left/bottom/right must also be 16px. Asymmetric shorthand (`12px 16px`) is acceptable only when horizontal content needs more room. Four distinct values is almost always a mistake.

### Border Radius Consistency
Define a single `--radius` base token and derive all variants with `calc()`. Sharper corners feel technical, rounder corners feel friendly — adjust the base value to set personality:
...
Changing `--radius` reshapes the entire UI. Don't mix systems — consistency creates coherence.
...
**Subtle single shadows** — Soft lift without complexity. A simple `0 1px 3px oklch(0 0 0 / 8%)` can be enough. Works for approachable products that want gentle depth without the weight of layered shadows.
...
Use OKLCH for all color values — it produces perceptually uniform results and is the current CSS ecosystem standard.
...
- Define easing as tokens: `--ease-out: cubic-bezier(0.25, 1, 0.5, 1)` and `--ease-snappy: cubic-bezier(0.2, 0, 0, 1)`
- No spring/bouncy effects in enterprise UI
- Always honor `prefers-reduced-motion: reduce` — disable transitions or reduce to instant (`0ms`)

### Contrast Hierarchy
Build a four-level system mapped to semantic tokens:

- **Foreground** (`var(--foreground)`) — Primary text, headings. Must meet WCAG AA 4.5:1 against background.
- **Secondary** (`var(--muted-foreground)`) — Supporting text, descriptions. Must meet 4.5:1 for body, 3:1 for large text.
- **Muted** — Placeholders, disabled labels. Allowed below 4.5:1 only for non-essential content.
- **Faint** — Dividers, subtle borders. Decorative only, no readability requirement.

Use all four consistently. Skipping levels creates flat, unreadable interfaces.
...
Accessibility is not a feature — it's part of the quality floor. These are non-negotiable minimums.

### Contrast Ratios (WCAG AA)
- **Body text:** 4.5:1 minimum against its background
- **Large text** (18px+ or 14px bold): 3:1 minimum
- **UI components and icons:** 3:1 against adjacent colors
- Use your semantic token pairs (`--primary` / `--primary-foreground`) to guarantee contrast by design

### Focus Indicators
Every interactive element must have a visible focus state. Use `:focus-visible` (not `:focus`) to avoid showing rings on mouse clicks:
...
Never remove outlines without providing an equivalent alternative.

### Reduced Motion
Respect user preferences. Wrap all transitions and animations:
...
- Follow WAI-ARIA patterns for custom components (combobox, dialog, menu, tabs)
- Ensure logical focus order matches visual order
- Trap focus inside modals and dialogs when open
...
**Structure:** Define all colors as CSS custom properties in `:root` (light) and override in `.dark` (or `[data-theme="dark"]`). Never use separate stylesheets, runtime conditionals, or inline ternaries for theme switching — one set of tokens, two value sets.
...
- Removing `:focus-visible` outlines without providing an alternative
