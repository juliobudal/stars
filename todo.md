...
- [ ] **C2** — Move broadcasts in `Rewards::RedeemService` + `Tasks::ApproveService` to `after_commit` (avoid broadcast on rolled-back state)
- [ ] **C3** — PIN brute-force: 6-digit PIN OR exponential backoff + lockout in `ProfileSessionsController`
